...
do
    ...
    if [ "$ablation" = "none" ];
    then
        $bbcache -n ${n} -v 10 -c 2 -r $r -p 1 $out $label $minor &
    elif [ "$ablation" = "priority" ];
    then
        $bbcache -n ${n} -v 10 -b -r $r -p 1 $out $label $minor &
    elif [ "$ablation" = "support" ];
    then
        $bbcache -n ${n} -v 10 -c 2 -r $r -p 1 -a 1 $out $label $minor &
    elif [ "$ablation" = "pmap" ];
    then
        $bbcache -n ${n} -v 10 -c 2 -r $r $out $label $minor &
    elif [ "$ablation" = "lookahead" ];
    then
        $bbcache -n ${n} -v 10 -c 2 -r $r -p 1 -a 2 $out $label $minor &
    elif [ "$ablation" = "identical" ];
    then
        $bbcache -n ${n} -v 10 -c 2 -r $r -p 1 $out $label
    fi
done
