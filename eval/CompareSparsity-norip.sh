...
if [ "$#" -ne 4 ]; then
    ...
    printf "e.g. ./CompareSparsity-norip.sh frisk frisk_sparsity.csv ./sbrlmod ../data/CrossValidation\n"
    exit
fi
...
do
    ...
    sbrl_acc=$(cat $temp_f | grep -i "test accuracy" | tail -n 1 | sed "s/.*=//g" | xargs)
