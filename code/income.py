...
din = os.path.join('..', 'data', 'income')
dout = os.path.join('..', 'data', 'income')
fdata = os.path.join(din, 'income.data')
ftest = os.path.join(din, 'income.test')
fnames = os.path.join(din, 'income.names')
fcomplete = os.path.join(din, 'income-filtered.csv')
fout = os.path.join(din, 'income.csv')

if not os.path.exists(din):
    os.mkdir(din)
if not os.path.exists(fdata):
    uroot = 'https://archive.ics.uci.edu/ml/machine-learning-databases/census-income-mld/'
    os.system('wget %scensus-income.data.gz -O %s.gz' % (uroot, fdata))
    os.system('wget %scensus-income.test.gz -O %s.gz' % (uroot, ftest))
    os.system('wget %scensus-income.names -O %s' % (uroot, fnames))
    os.system('gunzip %s.gz' % fdata)
    os.system('gunzip %s.gz' % ftest)

# MSA = metropolitan statistical area
names = ['age', 'worker-class', 'industry-code', 'occupation-code', 'education',
         'wage-per-hour', 'enrolled-last-wk', 'marital-stat', 'industry',
         'occupation', 'race', 'hispanic-origin', 'sex', 'union-member',
         'unemployment-reason', 'employment-stat', 'capital-gains', 'capital-losses',
         'stock-dividends', 'tax-filer-stat', 'previous-residence-region',
         'previous-residence-state', 'household-family-stat',
         'household-summary', 'instance-weight', 'migration-change-in-msa',
         'migration-chance-in-reg', 'migration-move-within-reg',
         'live-same-house-1ya', 'migration-prev-res-sunbelt',
         'num-working-for-employer', 'family-under-18', 'father-country',
         'mother-country', 'birth-country', 'citizenship',
         'own-business-self-employed', 'veterans-questionnaire',
         'veterans-benefits', 'num-weeks-worked', 'year', 'income']

z = open(fdata, 'rU').read().strip().replace(', ', ',').replace('- ', '-').replace(' ', '-')
f = open(fcomplete, 'w')
f.write(','.join(names) + '\n')
f.write(z)
f.close()
...
columns = [age, x['worker-class'], x['education'], x['enrolled-last-wk'],
           x['marital-stat'], x['industry'], x['occupation'], x['race'],
           x['hispanic-origin'], x['sex'], x['union-member'],
...
names = ['age', 'worker-class', 'education', 'enrolled-last-wk',
         'marital-stat', 'industry', 'occupation', 'race',
         'hispanic-origin', 'sex', 'union-member',
