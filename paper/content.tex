...
Let~$\Prefix$ and~$\tilde\Prefix$ two prefixes that are
equivalent up to a permutation of their antecedents,
such that their lower bounds are related by
${b(\Prefix, \x, \y) \le b(\tilde\Prefix, \x, \y)}$.
%
Let~$\Prefix^*$ be the optimal prefix that starts with~$\Prefix$,
\begin{align}
\Prefix^* = \argmin_{\Prefix' : \Prefix' \textnormal{ starts with } \Prefix}
b(\Prefix, \x, \y),
\end{align}
and let~$\tilde\Prefix^*$ be the optimal prefix that starts with~$\tilde\Prefix$,
\begin{align}
\Prefix^* = \argmin_{\Prefix' : \Prefix' \textnormal{ starts with } \Prefix}
b(\Prefix, \x, \y),
\end{align}
It follows that~$\tilde\Prefix^*$ cannot be superior to~$\cal{P^*}$:
\begin{align}
...
\begin{proof}
Since $\Prefix$ and~$\tilde\Prefix$ have the same antecedents,
