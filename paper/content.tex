...
A rule list~${\RL = (r_1, r_2, \dots, r_K, r_0)}$ is a ${(K+1)}$-tuple
consisting of~$K$ association rules followed by a default rule~$r_0$;
define the length of~$\RL$ to be~${K \ge 0}$.
%
...
A prefix~${\Prefix = (p_1, \dots, p_K)}$ is a $K$-tuple of antecedents.
%
We sometimes define a rule list with respect to its prefix,
\ie as~${\RL = (\RL_\Prefix, \RL_q, q_0, K)}$,
where~$\RL_\Prefix$ is the $K$-tuple of antecedents before the default rule,
$\RL_q$ is a $K$-tuple of label predictions (consequents) corresponding to the prefix,
and~$q_0$ is the default label prediction.
%
Also, we say that prefix~${\Prefix' = (p_1, \dots, p_K, \dots, p_{K'})}$
starts with prefix~${\Prefix = (p_1, \dots, p_K)}$, for any~${1 \le K \le K'}$.

A rule list~$r$ classifies a datum~$x_n$ by providing the label prediction~$q_k$
of the first rule~$r_k$ whose antecedent~$p_k$ is true for~$x_n$.
%
We say that antecedent~$p_k$ captures~$x_n$ in the context of rule list~$r$;
note that~$r_k$ can be the default rule~$r_0$.
%
A prefix captures those data that are captured by one of its antecedents;
for a rule list with prefix~$\RL_\Prefix$, data not captured by~$\RL_\Prefix$
are classified according to the default label prediction~$q_0$.

Finally, given training data~${(\x, \y)}$, a prefix~${\Prefix = (p_1, \dots, p_K)}$
implies a rule list~$\RL = (\RL_\Prefix, \RL_q, q_0, K)$,
where~${\RL_\Prefix = \Prefix}$, and the label predictions~$\RL_q$ and~$q_0$
