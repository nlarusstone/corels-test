...
\begin{align}
b(\Prefix, \x, \y) &= m_p(\Prefix, \RL_q, \x, \y) + \Reg K \nn \\
&\le  m_p(\Prefix', \RL'_q, \x, \y) + \Reg K' = b(\Prefix', \x, \y)
\le \Obj(\RL', \x, \y).
\label{eq:prefix-lb}
\end{align}
%
...
Let ${\RL^* = (\Prefix, \RL_q, q_0, K)}$ be the optimal rule list,
with objective~$\ell^*$:
\begin{align}
...
Now consider the rule list ${\RL =  (\Prefix', \RL'_q, q'_0, K-1)}$
derived from~$\RL^*$ by deleting rule~$r_i$,
therefore ${\Prefix' = (p_1, \dots, p_{i-1}, p_{i+1}, \dots, p_K)}$
and ${\RL'_q = (q_1, \dots, q_{i-1}, q'_{i+1}, \dots, q'_K)}$,
