...
of data captured by~$\Prefix$ and not captured by~$\PrefixB$,
...
of data captured by~$\PrefixB$ and not captured by~$\Prefix$,
and let us require that~${\Omega \le \Reg}$, \ie
\begin{align}
\Omega \equiv \frac{1}{N} \sum_{n=1}^N
  \neg\, \Cap(x_n, \Prefix)
...
is at most~$2\Reg$:
\begin{align}
\left | \min_{\RL' \in \StartsWith(\Prefix)} \Obj(\RL', \x, \y)
  - \min_{\RLB' \in \StartsWith(\PrefixB)} \Obj(\RLB', \x, \y)
  \right | \le 2 \Reg.
\label{eq:similar}
\end{align}
...
The largest discrepancy between the objectives of~$\RL'$ and~$\RLB'$
would occur if one misclassified all the data corresponding to
both~$\phi$ and~$\Phi$ while the other correctly classified this
same data, thus
\begin{align}
...
where~${\kappa^* = \kappa + K^* - K}$.
%
We claim that~$\RLB^*$ is an optimal rule list
with prefix constrained to start with~$\PrefixB$,
\begin{align}
\RLB^* \in \argmin_{\RLB^\dagger \in \StartsWith(\PrefixB)} \Obj(\RLB^\dagger, \x, \y).
\label{eq:similar-analogous-optimal}
\end{align}
...
and claim~\ref{eq:similar-analogous-optimal}
reduces to claim~\ref{eq:equiv-analogous-optimal},
...
and vice versa, we again have~\eqref{eq:similar-analogous-optimal}.
