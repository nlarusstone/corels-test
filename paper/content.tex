...
We present two theorems, based on the upper bounds on prefix length
from Theorems~\ref{thm:ub-prefix-length} and~\ref{thm:ub-prefix-specific}.
%
...
\begin{align}
n_j = \sum_{\Prefix \in \Queue} \one [ L(\Prefix) = j ]
\end{align}
...
%
Set~${K = \lfloor \CurrentObj / \Reg \rfloor}$.
%
...
\begin{align}
\Remaining(\CurrentObj, \Queue)
\le \sum_{j=1}^J n_j \left( \sum_{k=0}^{K-j} P(M-j, k) \right)
