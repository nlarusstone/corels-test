...
%
For all ${j \le K - 1}$, every antecedent~$p_j$ in~$\Prefix$ has support
...
\begin{align}
\Supp(p_j, \x, \given \Prefix) < 1 - \Supp(\Prefix^{j-1}, \x \given \Prefix) - \Reg,
\label{eq:ub-support}
\end{align}
where ${\Prefix^{j-1} = (p_1, \dots, p_{j-1})}$.
\end{theorem}
...
${\OptimalObj \equiv \min_{\RL^\dagger} \Obj(\RL^\dagger, \x, \y)}$
...
\ie ${\RL' \in \argmin_{\RL^\dagger} \Obj(\RL^\dagger, \x, y)}$,
