...
\begin{algorithm}[t!]
  \caption{Incremental objective lower bound~\eqref{eq:inc-lb} and objective functions~\eqref{eq:inc-obj}
    used in Algorithm~\ref{alg:incremental}, our incremental branch-and-bound procedure.}
\label{alg:incremental-functions}
\begin{algorithmic}
\normalsize
\Function{IncrementalLowerBound}{$\mathbf{v}, \y, N$}
 ...
 \Function{IncrementalObjective}{$\mathbf{u}, \mathbf{v}, \y, N$}
    ...
    \If {$n_f / n_g > 0.5$}
        \State \Return $(n_a - n_b) / N$ \Comment{Default rule misclassification error with label $1$}
    \Else
        \State \Return $n_b / N$ \Comment{Default rule misclassification error with label $0$}
    \EndIf
\EndFunction
\end{algorithmic}
\end{algorithm}

Algorithm~\ref{alg:incremental} integrates incremental computation
into our branch-and-bound optimization procedure, and introduces
a cache for storing prefixes and their objective lower bounds.
%
We write these incremental computations with respect to two functions
that we provide explicitly in Algorithm~\ref{alg:incremental-functions}.
