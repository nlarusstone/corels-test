...
%
We expand the first difference as
\begin{align}
...
&= \Loss_p(\Prefix', \Labels', \x, \y) + \Loss_0(\Prefix', \Default', \x, y)
  - \Loss_p(\Prefix, \Labels, \x, \y) - \Loss_0(\Prefix, \Default, \x, \y) \nn.
\end{align}
Similarly, we expand the second difference as
\begin{align}
...
To prove our claim relating the objectives of~$\RL$, $\RL'$,
$\RLB$, and~$\RLB'$, we show that the difference in the
misclassification errors of prefixes~$\Prefix'$ and~$\Prefix$
is the same as that between~$\PrefixB'$ and~$\PrefixB$.
%
