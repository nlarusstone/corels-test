...
\end{align}
If~$\Prefix$ and~$\PrefixB$ are composed of the same antecedents,
\ie the two prefixes are equivalent up to a permutation of
their antecedents, then~${\kappa = K}$ and~${\kappa^* = K}$.
%
In this case, every~${\RL'' \in \StartsWith(\Prefix)}$
...
Otherwise, $\Prefix$ and~$\PrefixB$ are not composed of the
same antecedents.
%
Define ${\phi = \{p_k : (p_k \in \Prefix) \wedge (p_k \notin \PrefixB)\}}$
to be the set of antecedents in~$\Prefix$ but not in~$\PrefixB$,
and define ${\Phi = \{P_k : (P_k \in \PrefixB) \wedge (P_k \notin \Prefix)\}}$
to be the set of antecedents in~$\PrefixB$ but not in~$\Prefix$.
%
In this case, ${\phi \neq \emptyset}$ or ${\Phi \neq \emptyset}$, or both.
%
Suppose~${\phi \neq \emptyset}$, and let~${p \in \phi}$ be an
antecedent in~$\phi$, then there exists a set of rule lists
in~$\StartsWith(\PrefixB)$ that do not have analogues in~$\StartsWith(\Prefix)$.
%
Let~${\RLB'' \in \StartsWith(\PrefixB)}$ be such a rule list,
\ie its prefix ${\PrefixB'' = (P_1, \dots, P_\kappa, \dots, p, \dots)}$
starts with~$\PrefixB$, and among its remaining antecedents
contains at least one antecedent~${p \in \phi}$.
%
Since $\rho$ captures a subset of the data that~$\Prefix$ captures,
and~$\PrefixB$ captures the same data as~$\Prefix$,
it follows that~$p$ doesn't capture any data in~$\PrefixB''$:
\begin{align}
...
then any rule list~${\RL'' \in \StartsWith(\Prefix)}$,
such that~$\RL''$ contains an antecedent~${P \in \Phi}$,
cannot be optimal, \ie
${\RL^* \notin \argmin_{\RL^\dagger \in \StartsWith(\Prefix)} \Obj(\RL^\dagger, \x, \y)}$.
%
It thus holds that every~${\RL'' \in \StartsWith(\Prefix)}$
has a direct analogue~${\RLB'' \in \StartsWith(\PrefixB)}$,
such that~$\RL''$ and~$\RLB''$ obey~\eqref{eq:equiv-analogous},
and vice versa, and thus~\eqref{eq:equiv-analogous-optimal}.
%
