...
\end{align}
%
Let ${\Prefix = (p_1, \dots, p_{K-1}, p_{K})}$ be a prefix
that rejects its last antecedent~$p_{K}$,
...
and furthermore, $\RLB$ cannot be optimal,
...
\end{align}
where the first inequality reflects the condition that
${\PrefixB^{K'-1} \in \StartContains(\Prefix^{K-1})}$,
which implies that the set of antecedents in~$\PrefixB^{K'-1}$
contains the set of antecedents in~$\Prefix^{K-1}$, and the equality
that immediately follows reflects the fact that~${P_{K'} = p_K}$.
%
...
Define~$\StartContains(\Prefix)$ as in~\eqref{eq:start-contains},
and let ${\RL = (\Prefix, \Labels, \Default, K)}$ be a rule list with
prefix ${\Prefix = (p_1, \dots, p_{K-1}, p_{K})}$,
and labels ${\Labels = (q_1, \dots, q_{K-1}, q_{K})}$, such that
the last antecedent~$p_{K}$ has insufficient accurate support, \ie
\begin{align}
\frac{1}{N} \sum_{n=1}^N \one [ \Cap(x_n, p_K \given \Prefix) \wedge (q_K = y_n) ]
\le \Reg.
\end{align}
where~$\Reg$ is the regularization parameter.
%
