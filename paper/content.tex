...
In this section, we use our upper bounds on
prefix length~(\S\ref{sec:ub-prefix-length})
to derive upper bounds on the number of remaining
prefix evaluations during branch-and-bound execution.
%
At any point during branch-and-bound execution, we can calculate an upper bound
on the number of remaining prefix evaluations from the current best
objective~$\CurrentObj$ and information about prefixes are planning to evaluate,
\ie prefixes in the queue~$\Queue$ of Algorithm~\ref{alg:branch-and-bound}.

\subsubsection{Upper bounds on the number of prefix evaluations}
\label{sec:ub-size}

We present three theorems \dots
%
...
Let our state space correspond to rule lists formed from a set of~$M$ rules.
%
Let~$\MaxRemaining$ be the maximum number of prefixes evaluated by
Algorithm~\ref{alg:branch-and-bound}.
\begin{align}
\MaxRemaining = \sum_{k=1}^K P(M, k) = \sum_{k=1}^K \frac{M!}{(M - k)!},
\label{eq:size-naive}
\end{align}
...
%
Once we've evaluated the empty prefix, we'll have~${\CurrentObj \le 0.5}$,
...
%
We can think of our problem as finding the optimal $k$-permutation of
the~$M$ rules, where~${k \le K}$.
%
This gives
\end{proof}
...
%
For any length-$j$ prefix~$\Prefix$ in the queue, the maximum number
of prefixes that start with~$\Prefix$ and remain to be evaluated is:
\begin{align}
