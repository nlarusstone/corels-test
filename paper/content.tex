...
%
Next, in Theorem~\ref{thm:default-similar}, we
make the last of the above conditions precise,
and then in Theorem~\ref{thm:reject}, we describe large classes of
related rule lists whose prefixes all reject the same antecedent.

\begin{theorem}[Lower bound on rule similarity to a default rule]
\label{thm:default-similar}
Let ${\RL = (\Prefix, \Labels, \Default, K)}$ be a rule list with prefix
${\Prefix = (p_1, \dots, p_K)}$.
%
Let ${\RL' = (\Prefix', \Labels', \Default', K + 1)}$ be a similar
rule list whose prefix ${\Prefix' = (p_1, \dots, p_K, p_{K+1})}$
starts with~$\Prefix$ and ends with a new antecedent~$p_{K+1}$.
%
If~$p_{K+1}$, in the context of~$\Prefix'$,
captures nearly all data not captured by~$\Prefix$,
except for a fraction~$\epsilon$ upper bounded by the regularization parameter~$\Reg$,
\begin{align}
1 - \Supp(\Prefix, \x) - \Supp(p_{K+1}, \x, \given \Prefix') \equiv \epsilon \le \Reg,
\end{align}
then the objective of any rule list~$\RL''$ whose prefix starts with~$\Prefix'$
is at least as great as that of~$\RL$:
\begin{align}
...
\begin{comment}
\begin{proof}
The data not captured by~$\Prefix'$ has normalized support~${\epsilon \le \Reg}$,
\begin{align}
1 - \Supp(\Prefix', \x) = \epsilon = 1 - \Supp(\Prefix, \x) - \Supp(p_{K+1}, \x, \given \Prefix') \le \Reg.
\end{align}
%
Consider the majority and minority class labels, with respect to the
set of data not captured by~$\Prefix$;
let~$\rho$ be the fraction of data not captured by~$\Prefix$
that have this minority label.
%
It follows that the default rule misclassification error for~$\RL$
is given by~$\rho$, \ie ${\Loss_0(\Prefix, \Default, \x, \y) = \rho}$.
%
It is clear that if~${\rho \le \Reg}$, then any longer rule list
${\RL' = (\Prefix', \Labels', \Default', K')}$, whose prefix~$\Prefix'$
starts with~$\Prefix$ cannot achieve a smaller objective,
because of the penalty for longer prefixes:
\begin{align}
\Obj(\RL', \x, \y) = \Loss(\RL', \x, \y) + \Reg K'
= \Loss_p(\Prefix', \Labels', \x, \y) + \Loss_0(\Prefix', \Default', \x, \y) + \Reg K'
\end{align}
%
The rule lists~$\RL$ and~$\RL'$ are very similar:
their first~$K$ rules are exactly the same,
last rule of~$\RL'$ is similar to the default rule of~$\RL$,
and the default rule of~$\RL'$ has a small effect.
%
The best outcome for~$\RL'$ would occur if its misclassification error
were smaller than that of~$\RL$ by~$\epsilon$:
\begin{align}
\Obj(\RL', \x, \y) = \Loss(\RL', \x, \y) + \Reg (K+1)
= \Loss(\RL, \x, \y) + \Reg(K+1)
= 
...
all data not captured by~$\Prefix$, \ie if both its last rule,
with antecedent~$p_{K+1}$, and its default rule, incurred zero
misclassification error.
%
This would happen if all data captured by~$p_K$ had the same class label,
while all data not captured by~$\Prefix'$ had the same, and thus opposite,
class label.
%
It follows that the objective of~$\RL'$ is
\begin{align}
\Obj(\RL, \x, \y) = \Loss(\RL, \x, \y) + \Reg K
&= \Loss_p(\Prefix, \Labels, \x, \y) + \Loss_0(\Prefix, \Default, \x, y) + \Reg K \nn \\
&\le \Loss_p(\Prefix, \Labels, \x, \y) + \frac{1}{2} \left(1 - \Supp(\Prefix, \x)\right) + \Reg K 
\end{align}
\begin{align}
\Obj(\RL', \x, \y) = \Loss(\RL', \x, \y) + \Reg (K + 1)
&= \Loss_p(\Prefix', \Labels', \x, \y) + \Loss_0(\Prefix', \Default', \x, \y) + \Reg (K + 1) \nn \\
&\ge \Loss_p(\Prefix, \Labels, \x, \y) + \Reg (K + 1)
\end{align}
\end{proof}
...
\begin{proposition}[Propagation of insufficient antecedent support]
