...
\begin{align}
\Supp(p_K, \x, \given \Prefix) \le 1 - \Supp(\Prefix^{K-1}, \x \given \Prefix) - \Reg,
...
\begin{align}
\Supp(p_k, \x, \given \Prefix) < 1 - \Supp(\Prefix^{k-1}, \x \given \Prefix) - \Reg,
\label{eq:ub-support}
\end{align}
where ${\Prefix^{k-1} = (p_1, \dots, p_{k-1})}$.
\end{theorem}
...
or excessive support (Theorem~\ref{thm:ub-support}),
...
and let ${\RLB = (\PrefixB, \LabelsB, \DefaultB, K')}$
be any rule list with prefix
${\PrefixB = (P_1, \dots, P_{K'-1}, P_{K'})}$
such that~$\PrefixB$ starts with ${\PrefixB^{K'-1} =}$
${(P_1, \dots, P_{K'-1}) \in \StartContains(\Prefix^{K-1})}$
and ends with~${P_{K'} = p_{K}}$.
%
...
\begin{proof}
The support of~$p_K$ in~$\Prefix$ depends only
on the set of antecedents in~$\Prefix^{K-1}$:
\begin{align}
\Supp(p_K, \x \given \Prefix)
= \frac{1}{N} \sum_{n=1}^N \Cap(x_n, p_K \given \Prefix)
&= \frac{1}{N} \sum_{n=1}^N \left( \neg\, \Cap \left(x_n, \Prefix^{K-1} \right) \right)
  \wedge \Cap(x_n, p_K) \nn \\
&= \frac{1}{N} \sum_{n=1}^N \left( \bigwedge_{k=1}^{K-1} \neg\, \Cap(x_n, p_k) \right)
  \wedge \Cap(x_n, p_K)
\le \Reg,
\end{align}
\ie it does not depend on their order.
%
Similarly, the support of~$P_{K'}$ in~$\PrefixB$ depends only
on the set of antecedents in~$\PrefixB^{K'-1}$:
\begin{align}
\Supp(P_{K'}, \x \given \PrefixB)
= \frac{1}{N} \sum_{n=1}^N \Cap(x_n, P_{K'} \given \PrefixB)
&= \frac{1}{N} \sum_{n=1}^N \left( \neg\, \Cap \left(x_n, \PrefixB^{K'-1} \right) \right)
  \wedge \Cap(x_n, P_{K'}) \nn \\
&= \frac{1}{N} \sum_{n=1}^N \left( \bigwedge_{k=1}^{K'-1} \neg\, \Cap(x_n, P_k) \right)
   \wedge \Cap(x_n, P_{K'}) \nn \\
...
\begin{proposition}[Propagation of insufficient accurate antecedent support]
\label{prop:min-capture-correct}
Define~$\StartContains(\Prefix)$ as in~\eqref{eq:start-contains},
and let ${\RL = (\Prefix, \Labels, \Default, K)}$ be a rule list
with prefix ${\Prefix = (p_1, \dots, p_{K-1}, p_{K})}$
and labels ${\Labels = (q_1, \dots, q_{K-1}, q_{K})}$, such that
the last antecedent~$p_{K}$ has insufficient accurate support, \ie
\begin{align}
...
be any rule list with prefix
${\PrefixB = (P_1, \dots, P_{K'-1}, P_{K'})}$
and labels ${\LabelsB = (Q_1, \dots, Q_{K'-1}, Q_{K'})}$
such that~$\PrefixB$ starts with ${\PrefixB^{K'-1} =}$
${(P_1, \dots, P_{K'-1})}$ ${\in \StartContains(\Prefix^{K-1})}$
and ends with~${P_{K'} = p_{K}}$.
%
It follows that~$P_{K'}$ has insufficient accurate support in
prefix~$\PrefixB$, and furthermore, $\RLB$~cannot be optimal, \ie
${\RLB \notin}$ ${\argmin_{\RL^\dagger} \Obj(\RL^\dagger, \x, \y)}$.
\end{proposition}
...
%
For the next equality, notice that~$Q_{K'}$ is the majority class
label of data captured by~$P_{K'}$ in~$\PrefixB$, and~$q_K$ is
the majority class label of data captured by~$P_K$ in~$\Prefix$,
and recall from~\eqref{ineq:supp} that
${\Supp(P_{K'}, \x \given \PrefixB) \le \Supp(p_{K}, \x \given \Prefix)}$.
%
By Theorem~\ref{thm:min-capture-correct}, $\RLB$~cannot be optimal,
\ie ${\RLB \notin \argmin_{\RL^\dagger} \Obj(\RL^\dagger, \x, \y)}$.
\end{proof}

\begin{proposition}[Propagation of excessive antecedent support]
