...
for clarity, we sometimes call this a $K$-rule list.
%
...
A $k$-prefix ${p = (p_1, \dots, p_k)}$ is a $k$-tuple of antecedents.
%
For~$k \le K$, the $k$-prefix of a $K$-rule list~$\RL$
is a $k$-tuple of the first~$k$ antecedents in~$\RL$;
we refer to the $K$-prefix of a $K$-rule list simply as its prefix.
%
We often define a rule list with respect to its prefix as follows:
...
%
We say that prefix ${\Prefix' = (p_1, \dots, p_K, \dots, p_{K'})}$
starts with prefix ${\Prefix = (p_1, \dots, p_K)}$, for any~${1 \le K \le K'}$;
if~$\Prefix'$ starts with~$\Prefix$, then~$\Prefix'$ captures all the data
that~$\Prefix$ captures.

Finally, given training data~${(\x, \y)}$, a prefix ${\Prefix = (p_1, \dots, p_K)}$
implies a rule list ${\RL = (\Prefix, \RL_q, q_0, K)}$, where~${\Prefix = \Prefix}$,
and the label predictions ${\RL_q = (q_1, \dots, q_K)}$ and~$q_0$
are empirically set to minimize the number misclassification errors made
...
%
%In the remainder, we will sometimes suppress the dependence on training data in our notation.
%
...
First, define~${p_k(x_n \given p) = 1}$ if antecedent~$p_k$ captures~$x_n$
...
\begin{align}
b(\Prefix, \x, \y) \equiv m_p(\Prefix, \RL_q, \x, \y) + \Reg K \le \ell(\Prefix, \x, \y) = \ell(\RL, \x, \y),
...
%
Furthermore,~\eqref{eq:lower-bound} gives a lower bound on the objective of
\emph{any} rule list whose prefix starts with~$\Prefix$.

Notice that if~$\RL$ and~$\RL'$ are rule lists with prefixes~$\Prefix$
and~$\Prefixp$ such that~$\Prefixp$ starts with~$\Prefix$,
then ${m_p(\Prefixp, \RL_q, \x, \y) \ge m_p(\Prefix, \RL'_q, \x, \y)}$
because~$\Prefixp$ yields the same mistakes as~$\Prefix$, and possibly additional mistakes.
%
Therefore, ${b(\Prefixp, \x, \y) \ge b(\Prefix, \x, \y)}$.
%
...
objective~$\ell^*$; this is a dynamic, monotonically decreasing quantity.
%
