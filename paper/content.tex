...
For every prefix~$\Prefix$ evaluated during the execution of
Algorithm~\ref{alg:branch-and-bound}, we compute the objective
lower bound~${b(\Prefix, \x, \y)}$ and, sometimes,
...
%
Thus, the empty rule list's objective lower bound is
...
${\Obj(\RL, \x, \y) = b((), \x, \y) + \Loss_0((), \Default, \x, \y) = \Loss_0((), \Default, \x, \y)}$.

Here, we derive our incremental expressions for the objective function and its lower bound.
%
...
enforces that if we ever evaluate~$\RL'$, we will have already evaluated
both~$\RL$'s objective and objective lower bound.
%
...
with respect to the the objective lower bound of~$\RL$:
\begin{align}
b(\Prefix', \Labels', \x, \y)
...
&= \Loss_p(\Prefix, \Labels, \x, \y) + \Reg K + \Reg
  + \frac{1}{N} \sum_{n=1}^N \Cap(x_n, p_k \given \Prefix') \wedge \one [q_k \neq y_n ] \nn \\
...
only depends on~$\Prefix$, \ie it does not depend on~$\Prefix'$; furthermore,
...
Later, we describe our cache data structure for supporting
these incremental computations~(\S\ref{sec:cache}).
