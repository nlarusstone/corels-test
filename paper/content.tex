...
Theorem~\ref{thm:similar} tells us that if prefix~$\Prefix$
and~$\PrefixB$ are similar, and we know the objective of the optimal
rule list that starts with~$\Prefix$, then
\begin{align}
\min_{\RLB^\dagger \in \StartsWith(\PrefixB)} \Obj(\RLB^\dagger, \x, \y)
&\ge \min_{\RL^\dagger \in \StartsWith(\Prefix)} \Obj(\RL^\dagger, \x, \y)
...
%
During our rule list optimization procedure, if we demonstrate that
${\min_{\RL^\dagger \in \StartsWith(\Prefix)} \Obj(\RL^\dagger, \x, \y) \ge \CurrentObj}$,
