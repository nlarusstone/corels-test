...
\end{align}
This bound can be viewed as a generalization of~\eqref{eq:lookahead};
also note that when~$\Prefix$ is the empty prefix,
...
We can use our upper bounds on prefix length~(\S\ref{sec:ub-prefix-length})
to calculate upper bounds on the number of remaining prefix evaluations.
%
...
%
We illustrate the subsequent computational savings below in~\S\ref{sec:permutation-counting}.
