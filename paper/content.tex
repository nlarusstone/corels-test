...
&= \frac{1}{N} \sum_{n=1}^N \left( \one [ \neg\, \Cap(x_n, \Prefix) \wedge (q_0 \neq y_n) ]
  + \sum_{k=1}^K \one [ \Cap(x_n, p_k \given \Prefix) \wedge (q_k \neq y_n) ] \right)
  + \Reg K
\end{align}
...
${\Obj^* \equiv \Obj_- + \Obj_i + \Obj_+}$, where
\begin{align}
\Obj_- &= \frac{1}{N} \sum_{n=1}^N \sum_{k=1}^{i-1}
  \one [ \Cap(x_n, p_k \given \Prefix^k) \wedge (q_k \neq y_n) ] + \Reg (k - 1) \nn \\
\Obj_i &= \frac{1}{N} \sum_{n=1}^N
  \one [ \Cap(x_n, p_i \given \Prefix^i) \wedge (q_k \neq y_n) ] + \Reg \\
\Obj_+ &= \frac{1}{N} \sum_{n=1}^N \left( \one [ \neg\, \Cap(x_n, \Prefix) \wedge (q_0 \neq y_n) ]
  + \sum_{k=i+1}^K \one [ \Cap(x_n, p_k \given \Prefix^k) \wedge (q_k \neq y_n) ] \right)
  + \Reg (K - i) \nn
\end{align}
Now consider the rule list ${\RL =  (r_1, \dots, r_{i-1}, r_{i+1}, \dots, r_K, r_0)}$
