...
and let ${\Prefix = (p_1, \dots, p_K, p_{K+1})}$ be a prefix,
such that its last antecedent~$p_{K+1}$ has insufficient support,
\ie ${\Supp(p_K, \x \given \Prefix) \le \Reg}$,
where~$\Reg$ is the regularization parameter.
%
Let ${\RL' = (\Prefix', \Labels', \Default', K')}$
be any rule list in~$\StartContains(\Prefix)$,
and let~$p'_k$ be the antecedent in prefix
${\Prefix' = (p'_1, \dots, p'_k, \dots, p'_{K'})}$
such that ${p'_k = p_K}$.
%
It follows that prefix~$\Prefix'$ rejects~$p'_k$,
and furthermore, $\RL'$ cannot be optimal,
