...
\begin{lemma}[Generalized hierarchical objective lower bound]
\dots
\end{lemma}
...
In the following two corollaries, we use information about
the state of Algorithm~\ref{alg:branch-and-bound}'s execution
to calculate, for any given algorithm state,
...
Corollary~\ref{thm:ub-total-eval}, because it uses the current
best objective and information about the lengths of prefixes
in the queue to constrain the lengths of prefixes in the
...
We can use more fine-grained information about prefixes in the queue
to obtain a tighter upper bound on the number of remaining prefix evaluations.
%
Our next upper bound is strictly tighter than the bound in
Corollary~\ref{thm:remaining-eval-coarse}, because it additionally
incorporates prefix-specific objective lower bound information from
prefixes in the queue, which further constrains the lengths of
prefixes in the remaining search space.
