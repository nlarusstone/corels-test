...
Finally, given training data~${(\x, \y)}$, an antecedent list~$\Prefix$
implies a rule list ${\RL = (\Prefix, \Labels, \Default, K)}$,
with prefix~$\Prefix$, and where the label predictions $\Labels$
and~$\Default$ are empirically set to minimize the number of
misclassification errors made by the rule list on the training data.
%
