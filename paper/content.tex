...
In this section, we use our upper bounds on prefix length to derive upper bounds
on the number of prefix evaluations made by Algorithm~\ref{alg:branch-and-bound}.
%
First, in~\S\ref{sec:ub-size}, we present three bounds corresponding to
the three upper bounds on prefix length from~\S\ref{sec:ub-prefix-length}.
%
Next, in~\S\ref{sec:permutation-counting}, we present a bound
that accounts for the effect of symmetry-aware garbage collection.

\subsubsection{Upper bounds on prefix evaluations from upper bounds on prefix length}
\label{sec:ub-size}

Our first result below is a na\"ive upper bound on the total number of prefix evaluations.
%
For the following two theorems, we observe that at any point during
...
\begin{theorem}[Upper bound on the total number of prefix evaluations]
...
\begin{proof}
At the beginning of Algorithm~\ref{alg:branch-and-bound},
the queue~${\Queue = ((),)}$ contains a single element, the empty prefix.
%
Once we've evaluated the empty prefix, we'll have an upper bound
on the current best objective, ${\CurrentObj \le 1/2}$,
and the queue will contain~$M$ elements.
%
By~\eqref{eq:max-length}, ${K = \lfloor 1 / 2 \Reg \rfloor}$ gives
an upper bound on the length of any optimal rule list.
%
Notice now that we can think of our problem as finding the optimal
$k$-permutation of the~$M$ rules, where~${k \le K}$, therefore
\begin{align}
\TotalRemaining(\RuleSet) \le 1 + \sum_{k=1}^K P(M, k) = \sum_{k=0}^K \frac{M!}{(M - k)!}.
\end{align}
\end{proof}
...
\begin{proof}
Recall our upper bound on prefix length~\eqref{eq:max-length-prefix}
for prefixes~$\Prefix'$ that start with some prefix~$\Prefix$:
\begin{align}
...
\subsubsection{Upper bound on prefix evaluations with symmetry-aware garbage collection}
\label{sec:permutation-counting}

Since a prefix of length~$k$ belongs to an equivalence class of~$k!$
prefixes equivalent up to permutation, symmetry-aware garbage
collection~(\S\ref{sec:permutation}) dramatically prunes the search space.

Notice that Algorithm~\ref{alg:branch-and-bound} describes a
breadth-first exploration of the state space of rule lists.
%
Now suppose we integrate symmetry-aware garbage collection into
...
\begin{proof}
At the start of the algorithm we evaluate the empty prefix,
yielding an upper bound on the current best objective,
${\CurrentObj \le 1/2}$.
%
By~\eqref{eq:max-length}, ${K = \lfloor 1 / 2 \Reg \rfloor}$ gives
an upper bound on the length of any optimal rule list.
%
Next, we evaluate~$M$ prefixes of length~${k=1}$,
followed by~${P(M, 2)}$ prefixes of length~${k=2}$,
