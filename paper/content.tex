...
Next, we compare the difference in the objectives
of rule lists~$\RL'$ and~$\RL$ to the difference
in the objectives of rule lists~$\RLB'$ and~$\RLB$.
%
Let us expand the first difference as
\begin{align}
...
The prefixes~$\Prefix$ and~$\PrefixB$ capture similar sets of data.
%
Equivalently, the set of data that is not captured by~$\Prefix$
is similar to the set of data that is not captured by~$\PrefixB$.
%
In precise terms,
\begin{align}
\frac{1}{N} \sum_{n=1}^N \Cap(x_n, \Prefix) \oplus \Cap(x_n, \PrefixB) =
\frac{1}{N} \sum_{n=1}^N (\neg\, \Cap(x_n, \Prefix)) \oplus (\neg\, \Cap(x_n, \PrefixB))
= \omega + \Omega \le 2 \Reg,
\end{align}
