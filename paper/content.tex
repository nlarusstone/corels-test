...
%
If we encounter a prefix~$\Prefix$ such that its lower bound
obeys ${b(\Prefix, \x, \y) \ge \CurrentObj}$,
...
%
For such a rule list~$\RL'$, we can upper bound its objective:
${\Obj(\RL', \x, \y) \ge b(\Prefix, \x, \y) \ge \CurrentObj}$,
therefore~$\RL'$ cannot be optimal. \\
