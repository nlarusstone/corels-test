...
and let ${\RL' = (\Prefix', \Labels', \Default', K')}$ be
another rule list, such that~${K' \ge K}$ and prefix~$\Prefix'$
...
\begin{proof}
Since prefix~$\Prefix'$ contains prefix~$\Prefix$,
the antecedents in~$\Prefix$ correspond to a contiguous
subsequence of antecedents anywhere in~$\Prefix'$.
%
Let ${\Prefix = (p_1, \dots, p_K)}$ and ${\Labels = (q_1, \dots, q_K)}$.
%
Let ${K' = K_1 + K + K_2}$, such that
${\Prefix' = (p_{-(K_1 - 1)}, \dots, p_{-1}, p_0, p_1, \dots, p_K, p_{K+1}, \dots, p_{K + K_2})}$,
and similarly,
${\Labels' = (q_{-(K_1 - 1)}, \dots, q_{-1}, q_0, q_1, \dots, q_K, q_{K+1}, \dots, q_{K + K_2})}$.
%
Notice that \dots
%~$\Prefix'$ yields the same mistakes as~$\Prefix$,
%and possibly additional mistakes:
\begin{align}
\Loss_p(\Prefix', \Labels', \x, \y)
&= \frac{1}{N} \sum_{n=1}^N \sum_{k=-(K_1 - 1)}^{K + K_2}
  \one [ \Cap(x_n, p_k \given \Prefix') \wedge (q_k \neq y_n) ] \\
&\equiv \Loss_p^- + \Loss_p' + \Loss_p^+,
%+ \sum_{k=K+1}^{K'} \one [ \Cap(x_n, p_k \given \Prefix') \wedge (q_k \neq y_n) ] \right) \nn \\
%&\ge \Loss_p(\Prefix, \Labels, \x, \y),
\end{align}
where
\begin{align}
\Loss_p^- &= \frac{1}{N} \sum_{n=1}^N \sum_{k=-(K_1 - 1)}^0
  \one [ \Cap(x_n, p_k \given \Prefix') \wedge (q_k \neq y_n) ] \\
\Loss_p' &= \frac{1}{N} \sum_{n=1}^N \sum_{k=1}^K
  \one [ \Cap(x_n, p_k \given \Prefix') \wedge (q_k \neq y_n) ] \\
\Loss_p^+ &= \frac{1}{N} \sum_{n=1}^N \sum_{k=-(K + 1)}^{K + K_2}
  \one [ \Cap(x_n, p_k \given \Prefix') \wedge (q_k \neq y_n) ]
\end{align}
where we have used the fact that
${\Cap(x_n, p_k \given \Prefix') = \Cap(x_n, p_k \given \Prefix)}$
