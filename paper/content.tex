...
%
For any optimal rule list
${\OptimalRL \in \argmin_\RL \Obj(\RL, \x, \y)}$
there exists an optimal rule list
${\RL' \in \argmin_\RL \Obj(\RL, \x, \y)}$
such that ${L(\RL') \le M}$.
\end{proposition}

\begin{proof}
Let ${\RL = (\Prefix, \Labels, \Default, K)}$ be any rule list
whose prefix contains at least one repeated antecedent, \ie
${\Prefix = (p_1, \dots, p_k, \dots, p_{k'}, \dots, p_K)}$,
where ${p_{k'} = p_k}$, for any ${k < k' \le K}$.
%
It follows that~$p_{k'}$ doesn't capture any data in~$\Prefix$,
thus ${\Supp(p_{k'}, \x \given \Prefix) = 0}$.
%
Let ${\RL' = (\Prefix', \Labels', \Default', K - 1)}$ be the
rule list derived from~$\RL$ by deleting~$p_{k'}$, \ie
${\Prefix' = (p_1, \dots, p_k, \dots, p_{k'-1},}$ ${p_{k' + 1}, \dots, p_K)}$.
%
Since~$p_{k'}$ didn't capture any data in~$\Prefix$,
all the rules in~$\RL'$ behave the same as their analogues in~$\RL$, thus
${\Labels' = (q_1, \dots, q_k, \dots, q_{k'-1}, q_{k' + 1}, \dots, q_K)}$
and~${\Default' = \Default}$.
%
Consequently, $\RL$ and $\RL'$ have the same misclassification error,
\ie ${\Loss(\RL, \x, \y) = \Loss(\RL', \x, \y)}$.
%
Since~$\RL'$ is shorter than~$\RL$,
\begin{align}
\Obj(\RL, \x, \y) = \Loss(\RL, \x, \y) + \Reg K
&= \Loss(\RL', \x, \y) + \Reg K \nn \\
&\ge \Loss(RL', \x, \y) + \Reg (K - 1) = \Obj(\RL', \x, \y) \ge \OptimalObj,
\end{align}
where~$\Reg$ is the regularization parameter.
%
If~${\Reg > 0}$, then~$\RL$ can never be optimal
which implies that every optimal rule list
must be composed of distinct antecedents,
of which there can be a maximum of~$M$.
%
Otherwise, if~${\Reg = 0}$, then~$\RL$ can only be optimal
if~$\RL'$ is also optimal, which implies that
every optimal rule list is either composed of distinct antecedents
or can be used to generate an equivalent rule list composed of
...
%
Theorem~\ref{thm:permutation} below implies that~$\RL'$
also has the minimum objective over all rule lists whose
prefixes start with \emph{any} prefix in~$\Pi$.
