...
${b(\Prefix') \ge b(\Prefix) + \Reg}$.
%
Thus, even if we find that~${b(\Prefix) < \CurrentObj}$,
as long as~\eqref{eq:lookahead} is true,
then ${b(\Prefix') \ge \CurrentObj}$
and we can prune~$\Prefix'$.
%
Since we can prune any prefix~$\Prefix'$ longer than~$\Prefix$,
we can also prune~$\Prefix$.
\end{proof}
