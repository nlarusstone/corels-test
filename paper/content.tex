...
Our problem has rich structure beyond the objective lower bound in
Theorem~\ref{thm:bound}, including several instances of symmetries.
%
...
We begin with an immediate consequence of Theorem~\ref{thm:bound}, followed by \dots
...
Let~$L(d)$ be the length of rule list~$d$
and let~$\CurrentObj$ be the current best objective.
For all optimal rule lists ${\OptimalRL \in \argmin_d R(d)}$
\begin{align}
L(\OptimalRL) \le \left\lfloor \frac{\CurrentObj}{\Reg} \right\rfloor.
\label{eq:max-length}
\end{align}
Furthermore, if we ever encounter a perfect $K$-rule list~$\RL$
such that~${\Obj(\RL, \x, \y) = \Loss(\RL, \x, \y) + \Reg K = \Reg K}$,
then as we continue, we only ever have to consider shorter prefixes:
\begin{align}
L(\OptimalRL) = K - 1.
...
For all optimal rule lists ${\OptimalRL \in \argmin_d R(d)}$,
\begin{align}
L(\OptimalRL) \le \left\lfloor \frac{1}{\Reg} \right\rfloor.
\end{align}
\end{corollary}

\begin{proof}
Let ${d = ((), (), q_0, 0)}$ be the empty rule list;
it has objective ${\Obj(d) = \Loss(d, \x, \y) \le \frac{1}{2}}$,
...
\begin{align}
\MaxLength(\Prefix) \le \left\lfloor \frac{\CurrentObj - b(\Prefix, \x, \y)}{\Reg} \right\rfloor.
