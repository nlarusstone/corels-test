...
%
It additionally reorganizes the structure of Algorithm~\ref{alg:branch-and-bound}
to group together the computations associated with all children of a particular prefix.
%
	...
	\State $\Prefix \gets Q$.pop(\,) \Comment{Remove a prefix~$\Prefix$ from the queue}
        ...
        \For {$s$ in $\RuleSet$} \Comment{Evaluate all of $\Prefix$'s children}
            \If {$s$ not in $\Prefix$}
                ...
                \State $b(\PrefixB, \x, \y) \gets $ \Call{IncObjectiveLowerBound}{$b(\Prefix, \x, \y), s, \x, \y, \Reg$} \Comment{See~\eqref{eq:inc-lb}}
                \If {$b(\PrefixB, \x, \y) < \CurrentObj$} \Comment{\textbf{Bound}: Apply bound from Theorem~\ref{thm:bound}}
                    \State $\Obj(\RLB, \x, \y) \gets $ \Call{IncObjective}{$b(\PrefixB, \x, \y), s, \x, \y$} \Comment{See~\eqref{eq:inc-obj}}
