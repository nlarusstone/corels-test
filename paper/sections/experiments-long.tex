...
which our antecedent mining framework combines into ${M=156}$ antecedents,
on average (folds ranged from containing 155 to 157 antecedents).
%
We also consider a second, similar antecedent set in~\S\ref{sec:examples} and~\S\ref{sec:sparsity}.
...
To motivate the feature set we used in our analysis of the ProPublica dataset,
we first show example rule lists using slightly different feature set (Figure~\ref{fig:propublica}).
%
Here, the age categories ($<$25, 25-45, $>$45) come directly from the ProPublica dataset,
and we include race categories (African American, Asian, Caucasian, Hispanic,
...
of~197 antecedents, across folds.
%
...
%
Also, to explore the effect of a different discretization of the age category,
in our subsequent analysis, we chose a set of age ranges that was fine-grained
for younger individuals (18-20, 21-22, 23-25, 26-45, $>$45).

\begin{figure}[t!]
\begin{algorithmic}
\State \bif $(priors > 3)$ \bthen $yes$
\State \belif $(sex = male) \band (age < 25)$ \bthen $yes$
\State \belse $no$
\end{algorithmic}
\vspace{1mm}
\begin{algorithmic}
\State \bif $(age < 25) \band (priors = 2-3)$ \bthen $yes$
\State \belif $(age < 25) \band (juvenile~crimes > 0)$ \bthen $yes$
\State \belif $(priors > 3)$ \bthen $yes$
\State \belse $no$
\end{algorithmic}
\vspace{1mm}
\begin{algorithmic}
\State \bif $(juvenile~crimes = 0) \band (priors = 0)$ \bthen $no$
\State \belif $(sex = male) \band (age < 25)$ \bthen $yes$
\State \belif $(priors > 3)$ \bthen $yes$
\State \belse $no$
\end{algorithmic}
\caption{Three representative optimal rule lists that predict two-year recidivism for the
ProPublica dataset, found by CORELS, across 10 cross-validation folds.
%
The remaining seven optimal rule lists were the same or similar to the top or
middle of these, with prefixes containing the same rules, up to a permutation,
...
(${\Reg = 0.005}$, Figure~\ref{fig:recidivism-all-folds}),
have mean test accuracy~0.68, with standard deviation~0.02.
%
For comparison, the optimal rule lists in Figure~\ref{fig:propublica}
achieve a similar mean test accuracy of~0.66, with standard deviation~0.02.
...
%
Across the 10 folds, the COMPAS algorithm scores obtain
mean test accuracy of~0.66, with standard deviation~0.02.
