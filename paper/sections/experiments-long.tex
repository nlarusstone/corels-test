...
%
How does CORELS' predictive performance compare to that of other algorithms,
...
%
The optimal rule lists found using Feature Sets~A and~B are very similar.
%
The upper and lower rule lists are representative of seven and three of the folds, respectively.
%
Each of the remaining eight optimal rule lists is the same or similar to one these examples,
...
%
For smaller values of the regularization parameter, we observe less of this robustness,
as rule lists are allowed to grow in length.
%
We note that the prefix rules always predict the positive class label
...
width=0.9\textwidth]{figs/weapon-sparsity-jmlr.pdf}
\vspace{-4mm}

\end{center}
\caption{Predictive performance versus model size across different methods.
%
...
%
Top: Training and test accuracy as a function of model size,
...
%
Bottom: True positive rate (TPR, open markers) and false positive rate (FPR, solid markers)
for the test set, for weapon prediction with the NYCLU stop-and-frisk dataset.
%
...
Figure~\ref{fig:sparsity} summarizes differences in predictive performance and model size
for CORELS and other tree (CART, C4.5) and rule list (RIPPER, SBRL) learning algorithms.
%
Here, we vary different algorithm parameters, and increase the number of iterations for SBRL to 10,000.
%
For two-year recidivism prediction with the ProPublica dataset, we plot both training and test accuracy,
...
we plot both true positive rate (TPR) and false positive rate (FPR),
...
%
Both algorithms have much higher true positive rates (TPR's) and false positive rates (FPRs) for blacks than whites, and higher true negative rates (TNR's) and false negative rates (FNRs) for whites than blacks.
%
...
\centering
\begin{tabular}{l|c|c|c}
& Max evaluated & Lower bound & Predicted runtime \\
& prefix length & evaluations & \\
\hline
Our approach & 5  & $ 2.8 \times 10^7$ & $36$ seconds \\
Brute force & 5 & $2.5 \times 10^{10}$ & $3.3 \times 10^4$ s = $9.0$ hours \\
Brute force & 10 & $5.0 \times 10^{20}$ & $6.5 \times 10^{14}$ s = $21$ million years \\
Our approach (1984) & 5 & $2.8 \times 10^7$ & $1.2 \times 10^6$ s = $13.5$ days \\
\end{tabular}
...
%
This helps explain why neither our algorithm, nor other branch-and-bound variants, had not been developed before now.
