...
we first show example rule lists found by CORELS, for a slightly different feature set.
%
\begin{figure}[t!]
\begin{algorithmic}
...
\end{figure}
%
In Figure~\ref{fig:propublica}, the age categories ($<$25, 25-45, $>$45) come directly from the ProPublica dataset,
...
%
For comparison, the optimal rule lists summarized in Figure~\ref{fig:propublica}
...
%
As in Figure~\ref{weapon-reg-execution}, we dynamically and incrementally
calculate~$\lfloor \log_{10} \Remaining(\CurrentObj, \Queue) \rfloor$,
where~$\Remaining(\CurrentObj, \Queue)$
...
%
On the COMPAS dataset, with ${M = 156}$ antecedents, naively evaluating all prefixes of up to length~5 would
require evaluating $8.7 \times 10^{10}$ different prefixes.
%
