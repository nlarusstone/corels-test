...
\includegraphics[trim={30mm, 20mm, 30mm, 10mm},
width=\textwidth]{figs/weapon_execution_large-remaining-space.pdf}
\end{center}
\caption{Left: CORELS with (lines) and without
(dashes) the equivalent points bound (Theorem~\ref{thm:identical}),
for the ProPublica dataset (${\Reg = 0.005}$, ${M = 155}$).
%
Right: CORELS with (lines) and without (dashes)
the symmetry-aware map %(\ie the permutation bound, Corollary~\ref{thm:permutation}),
for the NYCLU dataset (${\Reg = 0.01}$, ${M = 46}$).
%
