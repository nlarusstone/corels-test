...
width=0.95\textwidth]{figs/compare-weapon.pdf}
\end{center}
...
width=0.75\textwidth]{figs/weapon-sparsity-tpr.pdf}
\includegraphics[trim={17mm, 10mm, 27mm, 4mm},
width=0.75\textwidth]{figs/weapon-sparsity-fpr.pdf}
\end{center}
...
In prior work, we used CORELS to identify a short rule list depending on only
three features, age, prior convictions, and sex \citep{AngelinoLaAlSeRu17-kdd},
that achieves test accuracy comparable to COMPAS.
%
\citet{Dressel2018} reported a similar result for another simple classifier,
a logistic regression model with two features (age and prior convictions).
%
If we restrict CORELS to search the space of rule lists formed from these same
two features (${\Reg=0.005}$), it identifies the same rule list across 10-fold
cross-validation experiments (Figure~\ref{fig:compas-two-features}).
%
This optimal rule list achieves test accuracy that is again comparable to COMPAS.
%
Such a rule list is simpler to use than a linear model, which requires multiplication and addition operations to evaluate.
