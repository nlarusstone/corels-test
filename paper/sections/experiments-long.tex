...
%
In prior work, we used CORELS to identify a short rule list depending on only three features, age, prior convictions, and sex \citep{AngelinoLaAlSeRu17-kdd}, that achieves test accuracy comparable to COMPAS.
%
\citet{Dressel2018} reported a similar result for another simple classifier, a logistic regression model with two features (age and prior convictions).
%
If we restrict CORELS to search the space of rule lists formed from these same two features (${\lambda=0.005}$), the optimal rule lists it finds achieve test accuracy that is again comparable to COMPAS.
%
CORELS identifies the same rule list across 10-fold cross-validation experiments.
%
