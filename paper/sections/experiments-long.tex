...
in New York City's stop-and-frisk policy.
%
We compare directly to their heuristic models in Section~\ref{sec:frisk}.
...
We showed examples of CORELS's models in Figures~\ref{fig} describe similarities and differences between models [??FIXME??]
...
%
The top subfigure plots the fraction of weapons recovered as a function of the fraction of
...
%
More generally, CORELS's all recover at least ${40\%}$ of weapons
on average, \ie more weapons than any of the heuristic models with~${T \ge 2}$,
which recover less than 25\% of weapons on average.
%
At the same time, CORELS's models all require well under 25\% of stops---significantly
...
The bottom subfigure plots both TPR and FPR as a function of model size,
for each of the models in the top subfigure.
%
For the heuristic, we define the model size as the number of model parameters;
...
between~3 and approximately~5 rules, on average.
%
CORELS's models are thus approximately as small, interpretable, and transparent
as the heuristic models; furthermore, their predictions are straightforward
to compute, and do not even require arithmetic.
...
across~10 cross-validation folds.
%
Note that the fraction of weapons recovered (top) is equal to the TPR (bottom).
%
...
%
Horizontal tick labels correspond to the legend in the top~figure.
}
\label{fig:frisk}
\end{figure}
