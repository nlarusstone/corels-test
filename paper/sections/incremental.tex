...
\State \textbf{Output:} Component of~$\RLB$'s misclassification error due to its default rule \\
 ...
 \Function{IncrementalObjective}{$\mathbf{u}, \mathbf{v}, \y, N$}
    ...
    \If {$n_g / n_f > 0.5$}
        \State \Return $(n_f - n_g) / N$ \Comment{Misclassification error of default label prediction $1$}
    \Else
        \State \Return $n_g / N$ \Comment{Misclassification error of default label prediction $0$}
    \EndIf
\EndFunction
\end{algorithmic}
\end{algorithm}
