...
of the computational details from CORELS' inner loop, and highlight each of
...
%    \If {$s$ not in $\Prefix$}
       ...
       \EndIf
       \If {$n_w / n_v > 0.5$}
           \State $\delta_b \gets (n_v - n_w) / N$ \Comment{Misclassification error of the rule $s \rightarrow 1$}
       \Else
           \State $\delta_b \gets n_w / N$ \Comment{Misclassification error of the rule $s \rightarrow 0$}
       \EndIf
