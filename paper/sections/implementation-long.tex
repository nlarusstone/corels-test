...
\For {$s$ in $\RuleSet$} \Comment{Evaluate all of $\Prefix$'s children}
    \If {$s$ not in $\Prefix$}
       ...
       \If {$\Obj(\RLB, \x, \y) < \CurrentObj$}
        ...
        \EndIf
        \State $b_0(\PrefixB, \x, \y) \gets \Count(\mathbf{f} \wedge \mathbf{z})$ \Comment{Lower bound on the default rule}
        ...
        \EndIf
        \State \Call{CheckSymmetryAwareMap}{$\PrefixB, b$} \Comment{\textbf{Permutation bound (Corollary~\ref{thm:permutation})}}
...
\begin{algorithm}[t!]
  \caption{Symmetry-aware map queries, to support search space pruning triggered by
  the permutation bound (Corollary~\ref{thm:permutation}). For further context, see Algorithm~\ref{alg:bounds}.}
\label{alg:pmap}
\begin{algorithmic}

\Function{CheckSymmetryAwareMap}{$\PrefixB, b$}
\EndFunction
\end{algorithmic}
\end{algorithm}
