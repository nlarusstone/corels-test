...
these 28 dimensions, \eg (\emph{stop reason = suspicious object}).
%
...
\section*{Appendix D. Optimal rule lists for recidivism prediction, for different~$\Reg$}

Example optimal rule lists that predict two-year recidivism, for the ProPublica dataset,
found by CORELS, across 10 cross-validation folds.
%
Figures~\ref{fig:recidivism-rule-list-02} and~\ref{fig:recidivism-rule-list-01}
use regularization parameters~${\Reg = 0.02}$ and~0.01, respectively,
and Figure~\ref{fig:recidivism-all-folds}~(\S\ref{sec:examples}) uses~${\Reg = 0.005}$;
together, these also correspond to the experiments summarized in~\S\ref{sec:sparsity}
(Figure~\ref{fig:sparsity}, top).
%
The optimal rule lists tend to grow in length as the regularization parameter decreases.

For the largest regularization parameter~${\Reg = 0.02}$~(Figure~\ref{fig:recidivism-rule-list-02}),
we observe that all folds identify the same length-1 rule list.
%
For the intermediate value~${\Reg = 0.01}$~(Figure~\ref{fig:recidivism-all-folds}),
we observed in~\S\ref{sec:examples} that across the folds,
...
%
With the smaller regularization parameter~${\Reg = 0.005}$~(Figure~\ref{fig:recidivism-all-folds}, \S\ref{sec:examples}),
...
\State \belse $no$
\end{algorithmic}
\caption{Optimal rule list for the ProPublica dataset,
found by CORELS with regularization parameter~${\Reg = 0.02}$,
for all 10 cross-validation folds.
}
\label{fig:recidivism-rule-list-02}
\end{figure}
...
found by CORELS with regularization parameter~${\Reg = 0.01}$,
across 10 cross-validation folds.
}
\label{fig:recidivism-rule-list-01}
\end{figure}

\section*{Appendix E. Optimal rule lists for weapon prediction, for different~$\Reg$}
...
and correspond to the experiments summarized in~\S\ref{sec:sparsity}
(Figure~\ref{fig:sparsity}, bottom).
%
The optimal rule lists tend to grow in length as the regularization parameter decreases.
