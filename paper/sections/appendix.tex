...
and Figure~\ref{fig:recidivism-all-folds}~(\S\ref{sec:examples}) for~${\Reg = 0.005}$;
together, these correspond to the results for CORELS summarized in~\S\ref{sec:sparsity}
(Figure~\ref{fig:sparsity}, top).
%
Notice that optimal rule lists tend to grow in length as the regularization parameter decreases.
...
and Figure~\ref{fig:weapon-rule-list-0025} shows~0.0025;
together, these results correspond to the results for CORELS summarized
in~\S\ref{sec:sparsity} (Figure~\ref{fig:sparsity}, bottom).
%
The optimal rule lists tend to grow in length as the regularization parameter decreases.

For each of the larger regularization parameters in Figure~\ref{fig:weapon-rule-list-04-01},
