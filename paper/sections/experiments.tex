...
\begin{center}
% left lower right upper
\includegraphics[trim={25mm 10mm 35mm 15mm},
width=0.44\textwidth]{figs/weapon_reg_small-execution.pdf}
\end{center}
\caption{Example CORELS execution traces (NYCLU).
%
Objective value (lines) and lower bound (dashes) for CORELS.
%
...
%
Top: Objective value (thin lines) and lower bound (dashes) for CORELS,
as in Figure~\ref{fig:weapon-reg-execution},
and lower bound (thick lines) for separate executions of variants without
equivalent points (left) and permutation (right) bounds.
%
Bottom: Upper bound on remaining search space size,
for CORELS (thin lines) and variants (thick lines).
}
\label{fig:objective}
\end{figure}
