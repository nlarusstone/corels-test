...
\includegraphics[trim={10mm, 0mm, 20mm, 10mm}, width=0.45\textwidth]{figs/compas-sparsity-training.pdf}
\end{kdd}
\begin{arxiv}
...
NYCLU dataset.
%
Of the models shown, only CART with ${cp = 0.001}$ significantly overfits.
%
We depict neither RIPPER, which we were unable to execute,
nor C4.5, which finds large models (${>1000}$ leaves)
...
logistic regression, support vector machines, AdaBoost, CART, C4.5, random forests, RIPPER,~\footnote{We use publicly available R implementations, with default parameter settings.} and scalable rule lists (SBRL).~\footnote{https://github.com/Hongyuy/sbrlmod}
...
%
Figure~\ref{fig:rule-list} shows an example optimal rule list that CORELS learns
for predicting two-year recidivism, using the ProPublica dataset.
...
% left lower right upper
\includegraphics[trim={30mm 15mm 35mm 10mm}, width=0.45\textwidth]{figs/kdd_compas_compare_small-queue.pdf}
\end{kdd}
\end{center}
