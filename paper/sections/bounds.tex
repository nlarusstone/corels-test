...
%
The first two are found below, and the last in Appendix~A.
\end{proof}
...
combined with Proposition~\ref{prop:ub-support} (Appendix~A),
...
%
Consider a rule list starting with prefix~$\Prefix$,
that is the best possible prefix starting with~$\Prefix$.
%
If we take that rule list, and replace its prefix~$\Prefix$ with another prefix
whose support is the same as that of~$\Prefix$ (it could include different antecedents),
