...
class Node {
  public:
    ...
    Node(short id, size_t nrules, bool prediction, bool default_prediction,
    ...
    // node to the root
    inline std::pair<std::vector<short>, std::vector<bool>>
        get_prefix_and_predictions();

    inline size_t depth() const;
    inline Node<T>* child(short idx);
    inline Node<T>* parent() const;
    inline void delete_child(short idx);
    inline size_t num_children() const;

    inline T& get_storage(); // can this be const?
    inline size_t num_captured() const;

    inline typename std::map<short, Node<T>*>::iterator children_begin();
    inline typename std::map<short, Node<T>*>::iterator children_end();
    inline typename std::map<short, Node<T>*>::iterator random_child(); // FIXME
    // inline typename std::map<short, Node<T>*>::iterator random_child(PRNG prng);
  ...
  private:
    ...
    size_t depth_;
    Node<T>* parent_;
    std::map<short, Node<T>*> children_;
...
class CacheTree {
  public:
    ...
    inline std::vector<short> opt_rulelist() const;
    inline std::vector<bool> opt_predictions() const;

    inline size_t num_nodes() const;
    inline size_t num_evaluated() const;
    inline rule_t rule(short idx) const;
    inline char* rule_features(short idx) const;
    inline rule_t label(short idx) const;
    ...
    void update_opt_rulelist(std::vector<short>& parent_prefix,
                             short new_rule_id);
    ...
    N* check_prefix(std::vector<short>& prefix);
  ...
  private:
    ...
    std::vector<short> opt_rulelist_;
...
template<class T>
inline Node<T>* Node<T>::child(short idx) {
    typename std::map<short, Node<T>*>::iterator iter;
...
template<class T>
inline void Node<T>::delete_child(short idx) {
    children_.erase(idx);
}
...
template<class T>
inline typename std::map<short, Node<T>*>::iterator Node<T>::children_begin() {
    return children_.begin();
}

template<class T>
inline typename std::map<short, Node<T>*>::iterator Node<T>::children_end() {
    return children_.end();
}

template<class T>
inline typename std::map<short, Node<T>*>::iterator Node<T>::random_child() {
    typename std::map<short, Node<T>*>::iterator iter;
    short idx;
...
template<class N>
inline std::vector<short> CacheTree<N>::opt_rulelist() const {
...
template<class N>
inline rule_t CacheTree<N>::rule(short idx) const{
    return rules_[idx];
}

template<class N>
inline char* CacheTree<N>::rule_features(short idx) const{
    return rules_[idx].features;
}

template<class N>
inline rule_t CacheTree<N>::label(short idx) const{
    return labels_[idx];
}
